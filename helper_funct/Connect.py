...
from helper_funct import Exception as ex
...
class XTSConnect(XTSCommon):
    ...
    def __init__(self,
                 ...
                 pool=None,
                 proxies = None,
                 disable_ssl=False):  # default to False
        super().__init__()

        # Hardcoded defaults
        self._default_root_uri = "https://secure.aetramtrades.in"
        self._default_login_uri = self._default_root_uri + "/user/session"
        self._default_timeout = timeout or 7
        ...
        if proxies:
            self.reqsession.proxies.update(proxies)
    ...
    def get_order_book(self, clientID=None):
        """Request Order book gives states of all the orders placed by a user"""
        try:
            params = {}
    ...
    def convert_position(self, exchangeSegment, exchangeInstrumentID, targetQty, isDayWise, oldProductType,
        ...
        """Convert position API, enable users to convert their open positions from NRML intraday to Short term MIS or
    ...
    def get_order_history(self, appOrderID, clientID=None):
        """Order history will provide particular order trail chain. This indicates the particular order & its state
    ...
    def marketdata_login(self):
        try:
            ...
            if isinstance(response, dict) and "result" in response and "token" in response["result"]:
    ...
    def search_by_scriptname(self, scriptname):
        try:
            params = {"searchString": scriptname}
            response = self._get('market.search.instrumentsbystring', params)
            return response
        except Exception as e:
            print(f"Error in search_by_scriptname: {e}")
            return {"type": "error", "description": str(e), "result": []}
